...
class Meal(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'))
    calorie = db.Column(db.Integer, nullable=False, default=0)
    consume_date = db.Column(db.DateTime(timezone=True), nullable=False)
    def __repr__(self):
        return f"Goal(id={self.id}, calorie={self.calorie}, consume_date={self.consume_date}, user_id={self.user_id})"
...
def health_and_fitness():
    if request.method == 'POST':
        ...
        consumption_date = datetime.strptime(request.form.get('consume_date'), '%Y-%m-%d')
        user_id = current_user.id
        new_meal = Meal(calorie = calories, consume_date = consumption_date, user_id = user_id)
        db.session.add(new_meal)
        db.session.commit()
        flash('Meal tracked successfully!', category='success')
        return redirect(url_for('healthandfitness'))
    else:
        user_id = current_user.id
        user_meals = Meal.query.filter_by(user_id=user_id).all()
        return render_template('healthandfitness.html', meals=user_meals)

@app.route('/save_calorie_intake', methods=['POST'])
@login_required
def save_calorie_intake():
    if request.method == 'POST':
        calorie_intake = request.form.get('calories')
        user_id = current_user.id
        date = datetime.now()
        new_calorie_intake = healthandfitness(calorie_intake=calorie_intake, user_id=user_id, date=date)
        ...
        flash(f"On {date.strftime('%Y-%m-%d')}, you took in {calorie_intake} calories. Total: {total_calories} calories.", category='success')
